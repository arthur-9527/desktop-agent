...
class ActionExecutor:
    ...
    def update_os(self, os_type: str):
        ...
        if os_type != self.os_type:
            self.os_type = os_type
            self.key_mapper = KeyMapper(os_type)
            logger.info(f"[KeyMapper] 操作系统更新为: {os_type}")
    ...
    async def execute(self, parsed: dict, ime_status: str = ""):
        ...
        if action_type == "click":
            ...
            logger.info(f"[CheckInput] 结果: {result}")
            ...
            if raw_keys:
                ...
                logger.info(f"[Hotkey] {raw_keys} -> {keys}")
    ...
    async def _smart_type(self, content: str, mode: str = "replace", ime_status: str = "", press_enter: bool = False):
        ...
        if need_english_ime:
            if "中文" in ime_status:
                # 当前是中文模式，需要切换到英文
                logger.info(f"[SmartType] 当前中文模式，切换到英文")
                await self.client.keyboard_hotkey("ShiftLeft")
                await asyncio.sleep(0.2)  # 等待切换生效
            else:
                # 已经是英文模式或未知状态，假设可以输入
                logger.info(f"[SmartType] 输入法状态: {ime_status or '未知'}, 输入英文内容")
        else:
            # 中文内容
            if "英文" in ime_status:
                # 当前是英文模式，需要切换到中文
                logger.info(f"[SmartType] 当前英文模式，切换到中文")
                await self.client.keyboard_hotkey("ShiftLeft")
                await asyncio.sleep(0.2)
            else:
                logger.info(f"[SmartType] 输入法状态: {ime_status or '未知'}, 输入中文内容")

        # 3. 根据模式处理
        if mode == "replace":
            # Ctrl+A 全选，然后输入会自动替换
            logger.info(f"[SmartType] 模式=replace, 先全选再输入")
            await self.client.keyboard_hotkey("ControlLeft", "A")
            await asyncio.sleep(0.1)  # 短暂等待
        else:
            logger.info(f"[SmartType] 模式=append, 直接输入")

        # 4. 输入内容
        await self.client.keyboard_type(content)
        logger.info(f"[SmartType] 已输入: {content}")

        # 5. 等待输入完成（根据内容长度自动计算）
        #    每20个字符约10秒（0.5秒/字符）
        actual_wait = len(content) * 0.5
        logger.info(f"[SmartType] 等待输入完成: {actual_wait:.1f}s (内容长度={len(content)})")
        await asyncio.sleep(actual_wait)

        # 6. 如果 press_enter=true，在输入完成后按回车
        if press_enter:
            logger.info(f"[SmartType] 输入完成后按回车")
    ...
    async def ensure_english_input_method(self):
        ...
        """
        await self.client.keyboard_hotkey("ShiftLeft")
        logger.info("[InputMethod] 已切换输入法状态")